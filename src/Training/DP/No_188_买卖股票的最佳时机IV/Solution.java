package Training.DP.No_188_买卖股票的最佳时机IV;
...
class Solution {
    public int maxProfit(int k, int[] prices) {
         ...
         */
        if(k < 1) return 0;
        if(k >= prices.length/2) return greedy(prices);// k 太大无法设置 f 数组，会爆栈。等价于不限制交易次数。这样就不用设置 f 数组了 为什么是大于1/2是因为如果立马卖了就买可以或后面卖合并所以一次买卖必定有空一天，所以最多交易是1/2的长度
        int[][] t = new int[k][2];
        for(int i = 0; i < k; ++i)
